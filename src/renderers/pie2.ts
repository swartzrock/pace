...
class PieChart2 implements TimerRenderer {
	...
	render(details: TimerDetails): void {
		...
		const pieDetails: SquarePieChartDetails = { symbols: symbols, percentages: [details.percentDone, 1.0] }
		const squarePieChartTxt = this.pieChart.generate(pieDetails, radius, ' ', ' ')
		const pieChartTxt = Utils.horizDoubleTextBlock(squarePieChartTxt)
		let centeredPieChart = Utils.centerTextBlockHorizontallyOnScreen(pieChartTxt)
		...
		centeredPieChart = Utils.centerTextBlockInTextBlock(gradientTime, centeredPieChart)

		// Note - pie chart colorization must be done after the time has been rendered onto it
		centeredPieChart = centeredPieChart.replaceAll(this.CHART_FILL_CHAR, Colors.set(this.CHART_FILL_CHAR, fillColor))
		centeredPieChart = centeredPieChart.replaceAll(this.CHART_EMPTY_CHAR, Colors.set(this.CHART_FILL_CHAR, this.CHART_EMPTY_COLOR))
	...
	renderTimeRemainingFiglet(details: TimerDetails): string {
		...
		const timeRemainingPadded = Utils.setTextBlockPadding(timeRemainingFiglet, 1, 1, ' ')

		return timeRemainingPadded
	}

	calcFillColor(percentDone: number): Xterm256 {
		let index = 0
		if (percentDone > 0.9) {
			index = 2
